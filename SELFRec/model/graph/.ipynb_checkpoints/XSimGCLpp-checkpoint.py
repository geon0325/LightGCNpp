...
class XSimGCLpp(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(XSimGCLpp, self).__init__(conf, training_set, valid_set, test_set)
        self.cl_rate = conf.lmbda
        self.eps = conf.eps
        self.n_layers = conf.n_layer
        self.temp = conf.tau
        self.layer_cl = conf.l_star
        self.alpha = conf.alpha
        self.beta = conf.beta
        self.gamma = conf.gamma
        self.model = XSimGCL_Encoder(self.data, self.emb_size, self.eps, self.n_layers,self.layer_cl, self.alpha, self.beta, self.gamma)

        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_cl{self.cl_rate}_eps{self.eps}_nl{self.n_layers}_tau{self.temp}_lc{self.layer_cl}_alpha{self.alpha}_beta{self.beta}_gamma{self.gamma}'
        print()
...
class XSimGCL_Encoder(nn.Module):
    def __init__(self, data, emb_size, eps, n_layers, layer_cl, alpha, beta, gamma):
    ...
    def forward(self, perturbed=False):
        ...
        final_embeddings = (self.gamma * embs_zero) + ((1 - self.gamma) * embs_prop)
