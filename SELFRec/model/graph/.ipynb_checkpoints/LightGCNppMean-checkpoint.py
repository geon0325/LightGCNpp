...
class LightGCNppMean(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(LightGCNppMean, self).__init__(conf, training_set, valid_set, test_set)
        self.n_layers = conf.n_layer
        self.alpha = conf.alpha
        self.beta = conf.beta
        self.model = LGCN_Encoder(self.data, self.emb_size, self.n_layers, self.alpha, self.beta)
        
        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_nl{self.n_layers}_alpha{self.alpha}_beta{self.beta}'
        print()
...
class LGCN_Encoder(nn.Module):
    ...
    def forward(self):
        ...
        light_out = torch.mean(torch.stack(all_embeddings, dim=1), dim=1)
