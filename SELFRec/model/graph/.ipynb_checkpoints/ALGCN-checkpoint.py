...
class ALGCN(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(ALGCN, self).__init__(conf, training_set, valid_set, test_set)
        self.n_layers = conf.n_layer
        self.alpha = conf.alpha
        self.lmbda = conf.lmbda
        self.model = LGCN_Encoder(self.data, self.emb_size, self.n_layers, self.alpha)
        
        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_nl{self.n_layers}_alpha{self.alpha}_lmbda{self.lmbda}'
        print()
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                batch_loss = batch_loss + self.lmbda * cl_loss
    ...
    def cal_cl_loss(self, user_emb, item_emb):
        ...
        sim = torch.mm(user_emb_norm, item_emb_norm.T) / 0.1
...
class LGCN_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers, alpha):
        ...
        self.alpha = alpha
        self.norm_adj = data.normalize_graph_mat(data.ui_adj, self.alpha, 0.0)
