...
class NeuMF(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(NeuMF, self).__init__(conf, training_set, valid_set, test_set)
...
class Matrix_Factorization(nn.Module):
    def __init__(self, data, emb_size):
        ...
        self.user_layer = nn.Linear(emb_size, emb_size)
        self.item_layer = nn.Linear(emb_size, emb_size)
    ...
    def forward(self):
        return self.user_layer(self.embedding_dict['user_emb']), self.item_layer(self.embedding_dict['item_emb'])
