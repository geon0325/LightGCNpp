...
class DirectAU(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(DirectAU, self).__init__(conf, training_set, valid_set, test_set)
        #args = self.config['DirectAU']
        self.gamma = float(conf.gamma)
        self.n_layers= int(conf.n_layer)
        self.model = LGCN_Encoder(self.data, self.emb_size,self.n_layers)

        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_gamma{self.gamma}'
        print()
