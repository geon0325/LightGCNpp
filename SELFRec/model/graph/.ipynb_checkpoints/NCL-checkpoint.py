...
class NCL(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(NCL, self).__init__(conf, training_set, valid_set, test_set)
        ...
        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_ssl{self.ssl_reg}_prt{self.proto_reg}_tau{self.ssl_temp}_nl{self.n_layers}_hl{self.hyper_layers}_k{self.k}_nalpha{self.ncl_alpha}'
        print()
