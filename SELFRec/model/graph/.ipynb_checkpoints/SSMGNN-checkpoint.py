...
class SSMGNN(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(SSMGNN, self).__init__(conf, training_set, valid_set, test_set)
        self.n_layers = conf.n_layer
        self.alpha = conf.alpha
        self.beta = conf.beta
        self.tau = conf.tau
        self.model = LGCN_Encoder(self.data, self.emb_size, self.n_layers, self.alpha, self.alpha)
        
        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_nl{self.n_layers}_alpha{self.alpha}_beta{self.beta}_tau{self.tau}'
        print()
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                batch_loss = self.cal_cl_loss(user_emb, pos_item_emb) + l2_reg_loss(self.reg, model.embedding_dict['user_emb'][user_idx],model.embedding_dict['item_emb'][pos_idx],model.embedding_dict['item_emb'][neg_idx])/self.batch_size
    ...
    def cal_cl_loss(self, user_emb, item_emb):
        user_emb_norm = F.normalize(user_emb, p=2, dim=1)
        item_emb_norm = F.normalize(item_emb, p=2, dim=1)

        sim = torch.mm(user_emb_norm, item_emb_norm.T) / self.tau
        sim = F.softmax(sim, dim=1)

        return - torch.log(torch.diag(sim)).mean()
...
class LGCN_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers, alpha, beta):
