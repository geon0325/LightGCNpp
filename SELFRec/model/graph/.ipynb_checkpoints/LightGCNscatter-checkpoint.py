        ...
        for epoch in range(self.maxEpoch):
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                if n+1 % 100==0 and n>0:
                    print('training:', epoch + 1, 'batch', n, 'batch_loss:', batch_loss_emb.item())
...
class LGCN_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers, init):
        ...
        self.ui_adj = torch.sparse_coo_tensor(indices, values, torch.Size(coo.shape))
        self.indices = self.ui_adj._indices().cuda()
        self.row_indices = self.indices[0, :]
        self.col_indices = self.indices[1, :]
        self.values = self.ui_adj._values().cuda()
        self.rowsum = torch.sparse.sum(self.ui_adj, dim=1).to_dense().cuda()
        
        self.get_adj(self.alpha, self.beta)
    ...
    def get_adj(self, alpha, beta):
        ...
        scaled_values = self.values * d_inv_left[self.row_indices] * d_inv_right[self.col_indices]
        
        self.sparse_norm_adj = torch.sparse_coo_tensor(
            self.indices,
            scaled_values,
            self.ui_adj.size()
        ).cuda()
