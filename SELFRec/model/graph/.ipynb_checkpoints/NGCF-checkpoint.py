...
class NGCF(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(NGCF, self).__init__(conf, training_set, valid_set, test_set)
...
class NGCF_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers):
        ...
        self.W1 = nn.ModuleList([nn.Linear(emb_size, emb_size) for _ in range(n_layers)])
        self.W2 = nn.ModuleList([nn.Linear(emb_size, emb_size) for _ in range(n_layers)])
    ...
    def forward(self):
        ...
        for k in range(self.layers):
            emb1 = self.W1[k](ego_embeddings)
            emb1 = emb1 + torch.sparse.mm(self.sparse_norm_adj, emb1)
            emb2 = self.W2[k](ego_embeddings)
            emb2 = ego_embeddings * torch.sparse.mm(self.sparse_norm_adj, emb2)

            ego_embeddings = F.leaky_relu(emb1 + emb2)
            all_embeddings += [ego_embeddings]
        
        all_embeddings = torch.cat(all_embeddings, dim=1)

        user_mean_embeddings = all_embeddings[:self.data.user_num]
        item_mean_embeddings = all_embeddings[self.data.user_num:]
        
        return user_mean_embeddings, item_mean_embeddings, None, None
