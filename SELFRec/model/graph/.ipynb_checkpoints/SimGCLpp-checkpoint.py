...
class SimGCLpp(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(SimGCLpp, self).__init__(conf, training_set, valid_set, test_set)
        ...
        self.model = SimGCL_Encoder(self.data, self.emb_size, self.eps, self.n_layers, self.alpha, self.beta, self.gamma)
        
        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_cl{self.cl_rate}_eps{self.eps}_nl{self.n_layers}_alpha{self.alpha}_beta{self.beta}_gamma{self.gamma}'
        print()
...
class SimGCL_Encoder(nn.Module):
    def __init__(self, data, emb_size, eps, n_layers, alpha, beta, gamma):
        ...
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma # We don't use here.
