...
class NCLpp(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        ...
        self.config_name = f'{conf.dataset}_{conf.model_name}_seed{conf.seed}_lr{conf.learning_rate}_reg{conf.reg_lambda}_dim{self.emb_size}_ssl{self.ssl_reg}_prt{self.proto_reg}_tau{self.ssl_temp}_nl{self.n_layers}_hl{self.hyper_layers}_k{self.k}_nalpha{self.ncl_alpha}_alpha{self.alpha}_beta{self.beta}_gamma{self.gamma}'
        print()
    ...
    def ssl_layer_loss(self, context_emb, initial_emb, user, item):
        ...
        ssl_loss = self.ssl_reg * (ssl_loss_user + self.ncl_alpha * ssl_loss_item)
        return ssl_loss
...
class LGCN_Encoder(nn.Module):
    ...
    def forward(self):
        ...
        lgcn_all_embeddings = (self.gamma * embs_zero) + ((1 - self.gamma) * embs_prop)
