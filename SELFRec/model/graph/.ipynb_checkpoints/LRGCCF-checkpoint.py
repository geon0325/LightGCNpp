...
class LRGCCF(GraphRecommender):
    def __init__(self, conf, training_set, valid_set, test_set):
        super(LRGCCF, self).__init__(conf, training_set, valid_set, test_set)
        self.n_layers = conf.n_layer
        self.model = LRGCCF_Encoder(self.data, self.emb_size, self.n_layers)
...
class LRGCCF_Encoder(nn.Module):
    def __init__(self, data, emb_size, n_layers):
        super(LRGCCF_Encoder, self).__init__()
        self.data = data
        ...
        self.W = nn.ModuleList([nn.Linear(emb_size, emb_size) for _ in range(n_layers)])
    ...
    def forward(self):
        ...
        for k in range(self.layers):
            ego_embeddings = ego_embeddings + torch.sparse.mm(self.sparse_norm_adj, ego_embeddings)
            ego_embeddings = self.W[k](ego_embeddings)
            all_embeddings += [ego_embeddings]
